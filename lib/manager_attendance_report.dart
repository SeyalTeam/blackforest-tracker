...
class ManagerAttendanceReportScreen extends StatefulWidget {
  ...
  const ManagerAttendanceReportScreen({super.key, required this.managerCompanyIds});

  @override
  State<ManagerAttendanceReportScreen> createState() => _ManagerAttendanceReportScreenState();
}

class _ManagerAttendanceReportScreenState extends State<ManagerAttendanceReportScreen> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final results = await Future.wait([
        ApiService.instance.fetchAttendanceReport(startDate: dateStr, endDate: dateStr),
        ApiService.instance.fetchBranches(forceRefresh: true),
        ApiService.instance.fetchCompanies(),
      ]);

      setState(() {
        _attendanceReport = results[0] as Map<String, dynamic>;
  ...
  Map<String, List<Map<String, dynamic>>> _groupStatsByCompany() {
    final stats = _attendanceReport?['branchStats'] as List<dynamic>? ?? [];
    ...
    if (_isLoading) {
      ...
      if (groupedStats.isEmpty) {
        ...
        selectedStats.sort((a, b) {
          final amountA = (a['presentCount'] ?? 0).toDouble();
          final amountB = (b['presentCount'] ?? 0).toDouble();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Attendance Report', style: TextStyle(fontSize: 16)),
            Text(
              dateDisplay,
