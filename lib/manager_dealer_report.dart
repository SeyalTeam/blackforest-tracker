...
import 'branch_dealer_details.dart';

class ManagerDealerReportScreen extends StatefulWidget {
  final List<String> managerCompanyIds;
  const ManagerDealerReportScreen({super.key, required this.managerCompanyIds});

  @override
  State<ManagerDealerReportScreen> createState() => _ManagerDealerReportScreenState();
}

class _ManagerDealerReportScreenState extends State<ManagerDealerReportScreen> {
  bool _isLoading = true;
  String _errorMessage = '';
  
  Map<String, dynamic>? _dealerReport;
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final results = await Future.wait([
        ApiService.instance.fetchDealerReport(startDate: dateStr, endDate: dateStr),
        ApiService.instance.fetchBranches(forceRefresh: true),
        ApiService.instance.fetchCompanies(),
      ]);

      setState(() {
        _dealerReport = results[0] as Map<String, dynamic>;
  ...
  Map<String, List<Map<String, dynamic>>> _groupStatsByCompany() {
    final stats = _dealerReport?['groups'] as List<dynamic>? ?? [];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Dealer Report', style: TextStyle(fontSize: 16)),
            Text(
              dateDisplay,
