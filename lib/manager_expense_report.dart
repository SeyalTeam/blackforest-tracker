...
import 'branch_expense_details.dart';

class ManagerExpenseReportScreen extends StatefulWidget {
  final List<String> managerCompanyIds;
  const ManagerExpenseReportScreen({super.key, required this.managerCompanyIds});

  @override
  State<ManagerExpenseReportScreen> createState() => _ManagerExpenseReportScreenState();
}

class _ManagerExpenseReportScreenState extends State<ManagerExpenseReportScreen> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final results = await Future.wait([
        ApiService.instance.fetchExpenseReport(startDate: dateStr, endDate: dateStr),
        ApiService.instance.fetchBranches(forceRefresh: true),
        ApiService.instance.fetchCompanies(),
      ]);

      setState(() {
        _expenseReport = results[0] as Map<String, dynamic>;
  ...
  Map<String, List<Map<String, dynamic>>> _groupStatsByCompany() {
    final stats = _expenseReport?['groups'] as List<dynamic>? ?? [];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Expense Report', style: TextStyle(fontSize: 16)),
            Text(
              dateDisplay,
