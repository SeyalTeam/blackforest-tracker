...
import 'branch_stock_order_details.dart';

class ManagerStockOrderReportScreen extends StatefulWidget {
  final List<String> managerCompanyIds;
  const ManagerStockOrderReportScreen({super.key, required this.managerCompanyIds});

  @override
  State<ManagerStockOrderReportScreen> createState() => _ManagerStockOrderReportScreenState();
}

class _ManagerStockOrderReportScreenState extends State<ManagerStockOrderReportScreen> {
  bool _isLoading = true;
  String _errorMessage = '';
  
  Map<String, dynamic>? _stockOrderReport;
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final results = await Future.wait([
        ApiService.instance.fetchStockOrderReport(startDate: dateStr, endDate: dateStr),
        ApiService.instance.fetchBranches(forceRefresh: true),
        ApiService.instance.fetchCompanies(),
      ]);

      setState(() {
        _stockOrderReport = results[0] as Map<String, dynamic>;
  ...
  Map<String, List<Map<String, dynamic>>> _groupStatsByCompany() {
    final stats = _stockOrderReport?['stats'] as List<dynamic>? ?? [];
    ...
    for (var entry in grouped.entries) {
      final validBranches = entry.value.where((stat) => (stat['totalOrders'] as int? ?? 0) > 0).toList();
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final allDetails = (_stockOrderReport?['details'] as List<dynamic>? ?? []);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Stock Order Report', style: TextStyle(fontSize: 16)),
            Text(
              dateDisplay,
