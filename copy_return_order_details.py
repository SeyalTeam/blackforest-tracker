...
code = code.replace("'Expense Details'", "'Return Order Details'")

# Rewrite the ListView ItemBuilder completely
new_card = """
...
code = re.sub(r"              itemBuilder: \(context, index\) \{.*?              \},", new_card.strip(), code, flags=re.DOTALL)

with open('lib/branch_return_order_details.dart', 'w') as f:
    f.write(code)
